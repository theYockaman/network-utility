...
_wait_for_ip(){
  ...
  while [ $elapsed -lt $max_wait ]; do
    # Use precise matching with "/" followed by digits or space or end of line to avoid partial matches
...
if [ "$DRY_RUN" -eq 1 ]; then
  # In dry-run mode, check without sudo first, then try with sudo as fallback
